...
class KrakenExchange(Exchange):
    def __init__(self, options: ExchangeConfig) -> None:
        ...
        if options.trading_type == TradingType.LIVE:
            self._key, self._secret, self._passphrase = get_keys_from_environment('KRAKEN')
            self._client = API(self._key, self._secret)

        val = self._client.query_private('Balance', {}) if hasattr(self, '_client') else ['BACKTEST']
