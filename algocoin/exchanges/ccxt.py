...
from .helpers import CCXTHelpersMixin
...
class CCXTExchange(CCXTHelpersMixin, Exchange):
    def __init__(self, options: ExchangeConfig) -> None:
        super(CCXTExchange, self).__init__(options)
        self._type = ExchangeType.CCXT
        self._underlying_exchange = options.ccxt_exchange
        self._last = None
        self._orders = {}

        if options.trading_type == TradingType.LIVE or options.trading_type == TradingType.SIMULATION:
            self._key, self._secret, self._passphrase = get_keys_from_environment(self._underlying_exchange.value)
        elif options.trading_type == TradingType.SANDBOX:
            self._key, self._secret, self._passphrase = get_keys_from_environment(self._underlying_exchange.value + '_SANDBOX')

        if options.trading_type in (TradingType.LIVE, TradingType.SIMULATION, TradingType.SANDBOX):
            try:
                self._client = exchange_type_to_ccxt_client(self._underlying_exchange)({
                    'apiKey': self._key,
                    'secret': self._secret,
        ...
        self._subscription = [json.dumps({"type": "subscribe",
                                         "product_id": CCXTExchange.currencyPairToString(x)}) for x in options.currency_pairs]
    ...
    def buy(self, req: TradeRequest) -> TradeResponse:
        '''execute a buy order'''
        params = CCXTExchange.tradeReqToParams(req)
